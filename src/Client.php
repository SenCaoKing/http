...
class Client
{
    ...
    public function file($url, $field, $filename, array $data = array())
    {
        ...
        // PHP5.6 禁用了 '@/path/filename' 语法上传文件
     ...
     *      类似 'para1=var1&para2=val2&...'，此时Content-Type头将会被设置成"application/x-www-form-urlencoded"
     *      也可以使用一个以字段名为键值，字段数据为值的数组。
     *      如果value是一个数组，Content-Type头将会被设置成"multipart/form-data"
     *      从 PHP5.2.0 开始，使用 @ 前缀传递文件时，value 必须是个数组。
     *      从 PHP5.5.0 开始，@ 前缀已被废弃，文件可通过 \CURLFile 发送。
